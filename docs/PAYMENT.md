...
api-private-router includes an integrated payment system for self-service balance top-up.
...
If you previously used a separate payment service, you can move that workflow into api-private-router:

### Key Differences

| Aspect | Separate Payment Service | Integrated Payment |
|--------|-----------|-----------------|
| Deployment | Separate service (Next.js + PostgreSQL) | Built into api-private-router, no extra deployment |
| Payment Methods | EasyPay, Alipay, WeChat, Stripe | Same |
| Configuration | Environment variables + separate admin UI | Unified in api-private-router admin dashboard |
| Top-up Integration | Via Admin API callback | Internal processing, more reliable |
| Subscription Plans | Supported | Not yet (planned) |
| Order Management | Separate admin interface | Integrated in api-private-router admin dashboard |

### Migration Steps

1. Enable payment in api-private-router admin dashboard and configure providers (use the same payment credentials)
2. Update webhook callback URLs to api-private-router's callback endpoints
3. Verify that new orders are processed correctly through the integrated payment flow
4. Retire the separate payment service

> **Note**: Historical order data from the separate payment service will not be migrated automatically. Keep the previous service available for a period if you still need to query historical records.
