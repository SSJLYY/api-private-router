...
### 目标
本文档用于对接外部支付系统与 api-private-router 的 Admin API，覆盖：
- 支付成功后充值
- 用户查询
- 人工余额修正
- 前端购买页参数透传
...
```json
{
  ...
  "value": 100.0,
  "user_id": 123,
  "notes": "payment order: cm1234567890"
}
```
...
当 api-private-router 打开 `purchase_subscription_url` 或用户侧自定义页面 iframe URL 时，会统一追加：
- `user_id`
- `token`
- `theme`（`light` / `dark`）
...
### Purpose
This document describes the api-private-router Admin API endpoints used for external payment integrations, including:
...
- Preview: `http://<your-server-ip>:8084`
...
Note: Admin JWT can also access admin routes, but an Admin API Key is the preferred credential for server-to-server integration.
...
When api-private-router opens `purchase_subscription_url` or a user-facing custom page iframe URL, it appends:
- `user_id`
- `token`
- `theme` (`light` / `dark`)
...
### 6) `doc_url` guidance
- Point this field to your own deployment documentation
- Leave it empty if you do not want to expose a documentation entry in the frontend
