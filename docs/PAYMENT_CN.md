...
api-private-router 内置支付系统，支持用户自助充值，无需部署独立的支付服务。
...
> **易支付服务商选择建议**：兼容易支付协议的第三方聚合支付较多，建议重点核对以下信息后再接入：
>
> - 是否支持你当前需要的支付方式、币种与结算周期
> - 回调稳定性、文档完整度、签名机制与售后响应速度
> - 费率、提现门槛、实名认证或资质要求
> - 是否允许你的业务类型接入，并满足当地合规要求
>
...
如果你之前使用独立部署的外部支付系统，现在可以迁移到内置支付：

### 主要差异

| 对比项 | 旧外部支付系统 | 内置支付 |
|--------|-----------|---------|
| 部署方式 | 独立服务（Next.js + PostgreSQL） | 内置于 api-private-router，无需额外部署 |
| 支付方式 | EasyPay、支付宝、微信、Stripe | 相同 |
| 配置方式 | 环境变量 + 独立管理后台 | api-private-router 管理后台内统一配置 |
| 充值对接 | 通过 Admin API 回调 | 内部直接处理，更可靠 |
| 订阅套餐 | 视原系统而定 | 暂不支持（计划中） |
| 订单管理 | 独立管理界面 | 集成在 api-private-router 管理后台 |

### 迁移步骤

1. 在 api-private-router 管理后台启用支付并配置服务商（使用相同的支付凭证）
2. 更新 Webhook 回调地址为 api-private-router 的回调地址
3. 确认新订单通过内置支付正常处理
4. 停用旧外部支付服务

> **注意**：旧外部支付系统中的历史订单数据不会自动迁移。建议保留原系统一段时间以便查询历史记录。
