...
| 项目 | 说明 |
|------|------|
| **技术栈** | Java 后端 (Spring Boot) + Vue3 前端 (pnpm) |
| **数据库** | PostgreSQL 16 + Redis |
| **包管理** | 后端: Maven Wrapper, 前端: **pnpm**（不是 npm） |
...
```bash
# Java 17
# Maven Wrapper 已内置在 java-backend/.mvn/ 与 java-backend/mvnw(.cmd)
...
- 当前默认后端是 `java-backend/` 下的 Spring Boot 服务。
- 需要新增功能、修复线上问题或调整默认构建链路时，优先修改 `java-backend/`、`frontend/`、`deploy/` 与根目录 Java 相关脚本。
...
java -Dmaven.multiModuleProjectDirectory="$PWD" \
  ...
  org.apache.maven.wrapper.MavenWrapperMain test
...
psql -U api-private-router -h 127.0.0.1 -d api-private-router -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER api-private-router WITH PASSWORD 'api-private-router';
...
### 坑 6：Windows 上 psql 连 localhost 的 IPv6 问题
...
### 坑 7：Windows 没有 make 命令

**问题**：CI 里用 `make test-backend`，本地 Windows 没有 make。

**解决**：直接用 Makefile 里的原始命令：
```bash
# 代替 make test-backend
cd java-backend
java -Dmaven.multiModuleProjectDirectory=$PWD \
  -cp .mvn/wrapper/maven-wrapper.jar \
  org.apache.maven.wrapper.MavenWrapperMain test
...
### 坑 8：前端测试看似正常，但后端调用失败（模型映射被批量误改）
...
- [ ] `java ... MavenWrapperMain test` 通过
- [ ] `pnpm-lock.yaml` 已同步（如果改了 package.json）
- [ ] 所有 test stub 补全新接口方法（如果改了 interface）
- [ ] Java 端改动已完成对应测试/打包验证
...
```bash
# 连接数据库
psql -U api-private-router -h 127.0.0.1 -d api-private-router
...
# 执行 SQL 文件
psql -U api-private-router -h 127.0.0.1 -d api-private-router -f migration.sql
```
...
# 运行 Java 后端
cd java-backend
JAVA_TOOL_OPTIONS=-Dmaven.multiModuleProjectDirectory=$PWD \
java -cp .mvn/wrapper/maven-wrapper.jar org.apache.maven.wrapper.MavenWrapperMain spring-boot:run

# 运行测试
JAVA_TOOL_OPTIONS=-Dmaven.multiModuleProjectDirectory=$PWD \
java -cp .mvn/wrapper/maven-wrapper.jar org.apache.maven.wrapper.MavenWrapperMain test
```
...
api-private-router/
├── java-backend/
│   ├── src/main/java/       # Java 主后端源码
│   ├── src/main/resources/  # 配置与打包迁移脚本
│   └── pom.xml              # Maven 项目定义
├── frontend/
│   ├── src/
│   │   ├── api/             # API 调用
