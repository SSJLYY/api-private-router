<template>
  <BaseDialog
    :show="show"
    :title="t('admin.accounts.syncFromRemoteSourceTitle')"
    width="normal"
  ...
  >
    ...
    <form
      ...
      id="sync-from-remote-source-form"
      class="space-y-4"
      @submit.prevent="handlePreview"
    >
      <div class="text-sm text-gray-600 dark:text-dark-300">
        {{ t('admin.accounts.syncFromRemoteSourceDesc') }}
      </div>
      <div
        class="rounded-lg bg-gray-50 p-3 text-xs text-gray-500 dark:bg-dark-700/60 dark:text-dark-400"
      >
        {{ t('admin.accounts.remoteSourceUpdateBehaviorNote') }}
      </div>
      <div
        class="rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-600 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-400"
      >
        {{ t('admin.accounts.remoteSourceVersionRequirement') }}
      </div>

      <div class="grid grid-cols-1 gap-4">
        <div>
          <label for="external-source-base-url" class="input-label">{{ t('admin.accounts.externalSourceBaseUrl') }}</label>
          <input
            ...
            class="input"
            required
            :placeholder="t('admin.accounts.externalSourceBaseUrlPlaceholder')"
          />
        </div>

        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
            <label for="external-source-username" class="input-label">{{ t('admin.accounts.externalSourceUsername') }}</label>
            <input id="external-source-username" v-model="form.username" type="text" class="input" required autocomplete="username" />
          </div>
          <div>
            <label for="external-source-password" class="input-label">{{ t('admin.accounts.externalSourcePassword') }}</label>
            <input
              id="external-source-password"
    ...
    <div v-else-if="currentStep === 'preview' && previewResult" class="space-y-4">
      ...
      >
        <div class="mb-2 text-sm font-medium text-gray-700 dark:text-dark-300">
          {{ t('admin.accounts.remoteSourceExistingAccounts') }}
      ...
      <div v-if="previewResult.new_accounts.length">
        <div class="mb-2 flex items-center justify-between">
          <div class="text-sm font-medium text-gray-900 dark:text-white">
            {{ t('admin.accounts.remoteSourceNewAccounts') }}
          ...
          <div class="flex gap-2">
            <button
              ...
              @click="selectAll"
            >{{ t('admin.accounts.remoteSourceSelectAll') }}</button>
            <button
              type="button"
              class="text-xs text-gray-500 hover:text-gray-600 dark:text-gray-400"
              @click="selectNone"
            >{{ t('admin.accounts.remoteSourceSelectNone') }}</button>
          </div>
        </div>
        <div
        ...
        <div class="mt-1 text-xs text-gray-400">
          {{ t('admin.accounts.remoteSourceSelectedCount', { count: selectedIds.size }) }}
        </div>
      </div>
      ...
      >
        {{ t('admin.accounts.remoteSourceNoNewAccounts') }}
        <span v-if="previewResult.existing_accounts.length">
          {{ t('admin.accounts.remoteSourceWillUpdate', { count: previewResult.existing_accounts.length }) }}
        </span>
      </div>
    </div>
      ...
      <div class="flex justify-end gap-3">
        ...
        <template v-if="currentStep === 'input'">
          ...
          <button
            ...
            type="submit"
            form="sync-from-remote-source-form"
            :disabled="previewing"
          >
            {{ previewing ? t('admin.accounts.remoteSourcePreviewing') : t('admin.accounts.remoteSourcePreview') }}
          </button>
        </template>
        ...
        <template v-else-if="currentStep === 'preview'">
          <button
            ...
            @click="handleBack"
          >
            {{ t('admin.accounts.remoteSourceBack') }}
          </button>
          <button
            class="btn btn-primary"
            type="button"
...
import type { PreviewFromRemoteSourceResult } from '@/api/admin/accounts'
...
const previewResult = ref<PreviewFromRemoteSourceResult | null>(null)
const selectedIds = ref(new Set<string>())
const result = ref<Awaited<ReturnType<typeof adminAPI.accounts.syncFromRemoteSource>> | null>(null)
...
const handlePreview = async () => {
  ...
  try {
    const res = await adminAPI.accounts.previewFromRemoteSource({
    ...
    appStore.showError(error?.message || t('admin.accounts.remoteSourcePreviewFailed'))
  } finally {
...
const handleSync = async () => {
  ...
  syncing.value = true
  try {
    const res = await adminAPI.accounts.syncFromRemoteSource({
      base_url: form.base_url.trim(),
      username: form.username.trim(),
      password: form.password,
