export default {
  ...
  auth: {
    ...
    linuxdo: {
      ...
      backToLogin: '返回登录',
      invitationRequired: '该社区账号尚未注册，站点已开启邀请码注册，请输入邀请码以完成注册。',
      invalidPendingToken: '注册凭证已失效，请重新使用社区账号登录。',
    ...
    linuxdoCallbackPageTitle: '社区登录回调',
  ...
  admin: {
    ...
    backup: {
      ...
      r2Guide: {
        ...
        step1: {
          ...
          line2: '点击「创建存储桶」，输入名称（如 api-private-router-backups），选择区域',
          line3: '点击创建完成'
        },
        step2: {
          title: '创建 API 令牌',
    ...
    accounts: {
      ...
      syncFromCrsTitle: '从远程源同步账号',
      syncFromCrsDesc:
        '通过服务端到服务端连接，将远程源中的账号同步到当前系统。',
      crsVersionRequirement: '注意：远程源需要支持导出同步功能。',
      crsBaseUrl: '远程源地址',
      ...
      crsSelectAll: '全选',
      crsSelectNone: '全不选',
      crsNoNewAccounts: '所有远程源账号均已同步。',
      crsWillUpdate: '将更新 {count} 个已有账号。',
      crsSelectedCount: '已选择 {count} 个新账号',
      crsUpdateBehaviorNote:
        '已有账号仅同步远程源返回的字段，缺失字段保持原值；凭据按键合并；未勾选“同步代理”时保留原有代理。',
      crsBack: '返回',
      syncFromRemoteSourceTitle: '从远程源同步账号',
      syncFromRemoteSourceDesc: '通过服务端到服务端连接，将远程源中的账号同步到当前系统。',
      remoteSourceVersionRequirement: '注意：远程源需要支持导出同步功能。',
      externalSourceBaseUrl: '远程源地址',
      externalSourceBaseUrlPlaceholder: '例如：http://127.0.0.1:3000',
      externalSourceUsername: '用户名',
      externalSourcePassword: '密码',
      remoteSourcePreview: '预览',
      remoteSourcePreviewing: '预览中...',
      remoteSourcePreviewFailed: '预览失败',
      remoteSourceExistingAccounts: '将自动更新的已有账号',
      remoteSourceNewAccounts: '新账号（可选择）',
      remoteSourceSelectAll: '全选',
      remoteSourceSelectNone: '全不选',
      remoteSourceNoNewAccounts: '所有远程源账号均已同步。',
      remoteSourceWillUpdate: '将更新 {count} 个已有账号。',
      remoteSourceSelectedCount: '已选择 {count} 个新账号',
      ...
      poolModeInfo:
        '启用后，上游 429/403/401 错误将自动重试而不标记账号限流或错误，适用于上游指向另一个兼容中继实例的场景。',
    ...
    ops: {
      ...
      tooltips: {
        ...
        threads:
          'JVM/运行时线程数量。没有绝对"安全值"，建议以历史基线为准。经验参考：<2000 常见；2000-8000 需关注；>8000 且伴随队列上升时，优先排查阻塞/泄漏。',
    ...
    settings: {
      ...
      linuxdo: {
        title: '社区登录',
        description: '配置社区 OAuth，用于终端用户登录',
        enable: '启用社区登录',
        enableHint: '在登录/注册页面显示社区登录入口',
        clientId: 'Client ID',
        clientIdPlaceholder: '例如：hprJ5pC3...',
        clientIdHint: '从社区 OAuth 提供方后台获取',
        ...
        redirectUrlHint: '需与社区 OAuth 提供方中配置的回调地址一致（必须是 http(s) 完整 URL）',
      ...
      site: {
        ...
        siteSubtitlePlaceholder: '为团队提供统一 AI 接入',
        apiBaseUrl: 'API 端点地址',
      ...
      payment: {
        ...
        configGuide: '支付配置指南',
        configGuideHint: '开通方式、回调路径、支付路由与实例管理说明已整理在部署包文档中，建议结合当前环境配置核对。',
        enabled: '启用支付',
        enabledHint: '启用或禁用支付系统',
        enabledPaymentTypes: '启用的服务商',
        enabledPaymentTypesHint: '禁用服务商将同时禁用对应的实例。',
        findProvider: '如需接入易支付，请优先选择来源清晰、结算稳定且回调能力完整的服务商。',
        minAmount: '最低金额',
        maxAmount: '最高金额',
        dailyLimit: '每日限额',
      ...
      authSourceDefaults: {
        ...
        requireEmailHint: '启用后，社区登录、OIDC、微信注册缺少邮箱时必须先补充邮箱地址。',
        ...
        sources: {
          ...
          linuxdo: {
            title: '社区登录',
            description: '适用于社区第三方注册的新用户默认配额。'
          },
          oidc: {
            title: 'OIDC 登录',
  ...
  onboarding: {
    ...
    admin: {
      welcome: {
        title: '👋 欢迎使用 api-private-router',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">api-private-router 是一个灵活的 AI 接入平台，帮助您管理接入、路由与用量。</p><p style="margin-bottom: 12px;"><b>🎯 核心功能：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>📦 <b>分组管理</b> - 创建不同的访问层级与服务规则</li><li>🔗 <b>账号池</b> - 连接多个上游 AI 服务账号</li><li>🔑 <b>密钥分发</b> - 为用户生成独立的 API Key</li><li>💰 <b>计费管理</b> - 灵活的费率和配额控制</li></ul><p style="color: #10b981; font-weight: 600;">接下来，我们将用 3 分钟带您完成首次配置 →</p></div>',
        nextBtn: '开始配置 🚀',
        prevBtn: '跳过'
      },
      groupManage: {
        title: '📦 第一步：分组管理',
        description:
          '<div style="line-height: 1.7;"><p style="margin-bottom: 12px;"><b>什么是分组？</b></p><p style="margin-bottom: 12px;">分组是本项目中组织访问和计费规则的核心方式，类似一个“服务包”：</p><ul style="margin-left: 20px; margin-bottom: 12px; font-size: 13px;"><li>🎯 每个分组可以包含多个上游账号</li><li>💰 每个分组有独立的计费倍率</li><li>👥 可以设置为公开或专属分组</li></ul><p style="margin-top: 12px; padding: 8px 12px; background: #f0fdf4; border-left: 3px solid #10b981; border-radius: 4px; font-size: 13px;"><b>💡 示例：</b>您可以创建“优先访问”和“试用体验”等不同分组</p><p style="margin-top: 16px; color: #10b981; font-weight: 600;">👉 点击左侧的“分组管理”开始</p></div>'
      },
      createGroup: {
        title: '➕ 创建新分组',
        description:
    ...
    user: {
      welcome: {
        title: '👋 欢迎使用 api-private-router',
        description:
          '<div style="line-height: 1.8;"><p style="margin-bottom: 16px;">您好！欢迎来到 api-private-router。</p><p style="margin-bottom: 12px;"><b>🎯 快速开始：</b></p><ul style="margin-left: 20px; margin-bottom: 16px;"><li>🔑 创建 API 密钥</li><li>📋 复制密钥到您的应用</li><li>🚀 开始使用 AI 服务</li></ul><p style="color: #10b981; font-weight: 600;">只需 1 分钟，让我们开始吧 →</p></div>',
        nextBtn: '开始 🚀',
        prevBtn: '跳过'
      },
      keyManage: {
  ...
  leaderboard: {
    title: '消费排行榜',
    subtitle: '查看日榜、周榜、月榜、年度最佳员工，以及客户账号余额消耗排行。',
    backHome: '返回首页',
    backAdmin: '返回后台',
    loadError: '加载排行榜失败，请稍后重试。',
    empty: '暂无数据',
    userSectionTitle: '员工消费榜',
    userSectionHint: '年度最佳员工 = 本自然年累计第一',
    accountSectionTitle: '客户账号余额消耗榜',
    accountSectionHint: '优先使用账号口径成本，缺失时回退到实际扣费',
    daily: '日榜',
    weekly: '周榜',
    monthly: '月榜',
    yearlyBestEmployee: '年度最佳员工',
    consumption: '消费',
    accountConsumption: '余额消耗',
    totalPrefix: '总{metric}',
    userFallback: '用户 #{id}',
    accountFallback: '账号 #{id}',
    requestsTokens: '请求 {requests} · Tokens {tokens}',
    accountRequestsTokens: '{platform} · 请求 {requests} · Tokens {tokens}',
    actualCost: '实际扣费 {cost}',
    unknown: '未知',
    top8: '前 8 名',
    top12: '前 12 名',
    top20: '前 20 名',
