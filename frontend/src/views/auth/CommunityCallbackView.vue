    ...
    <div class="space-y-6">
      <div class="text-center">
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white">
          {{ t('auth.community.callbackTitle') }}
        </h2>
        <p class="mt-2 text-sm text-gray-500 dark:text-dark-400">
          {{ isProcessing ? t('auth.community.callbackProcessing') : t('auth.community.callbackHint') }}
        </p>
      </div>
        ...
        >
          ...
          <template v-if="needsInvitation">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              {{ t('auth.community.invitationRequired') }}
            </p>
            <div>
              <input
            ...
            >
              {{ isSubmitting ? t('auth.community.completing') : t('auth.community.completeRegistration') }}
            </button>
          </template>
          ...
          <template v-else-if="needsBindLogin">
            ...
            <div class="space-y-3">
              <input
                ...
                data-testid="community-bind-login-email"
                type="email"
                class="input w-full"
              ...
              <input
                ...
                data-testid="community-bind-login-password"
                type="password"
                class="input w-full"
              ...
              />
              <button
                data-testid="community-bind-login-submit"
                class="btn btn-primary w-full"
          ...
          <template v-else-if="needsTotpChallenge">
            ...
            <div class="space-y-3">
              <input
                v-model="totpCode"
                data-testid="community-bind-login-totp"
                type="text"
                inputmode="numeric"
                maxlength="6"
                class="input w-full"
              ...
              />
              <button
                data-testid="community-bind-login-totp-submit"
                class="btn btn-primary w-full"
...
type CommunityPendingActionResponse = PendingOAuthExchangeResponse & {
  step?: string
  intent?: string
  email?: string
...
function extractPendingAccountEmail(completion: CommunityPendingActionResponse): string {
  return (
...
function resolvePendingAccountAction(
  completion: CommunityPendingActionResponse
...
function applyPendingAccountAction(completion: CommunityPendingActionResponse) {
...
function applyTotpChallenge(completion: CommunityPendingActionResponse): boolean {
...
async function finalizeCompletion(completion: PendingOAuthExchangeResponse, redirect: string) {
  ...
  if (!isOAuthLoginCompletion(completion)) {
    throw new Error(t('auth.community.callbackMissingToken'))
  }
...
async function finalizePendingAccountResponse(completion: CommunityPendingActionResponse) {
...
async function handleSubmitInvitation() {
  ...
  if (!trimmedInvitationCode) return

  isSubmitting.value = true
  try {
    const affCode = loadOAuthAffiliateCode()
    const decision = currentAdoptionDecision()
    let completion: CommunityPendingActionResponse

    if (legacyPendingOAuthToken.value) {
      completion = (
        await apiClient.post<CommunityPendingActionResponse>(getCommunityOAuthCompleteRegistrationPath(), {
      ...
      completion = await completeCommunityOAuthRegistration(trimmedInvitationCode, decision, affCode)
    } else {
      completion = await completeCommunityOAuthRegistration(trimmedInvitationCode, decision)
    }
...
async function handleContinueLogin() {
  isSubmitting.value = true
  try {
    const completion = await exchangePendingOAuthCompletion(currentAdoptionDecision()) as CommunityPendingActionResponse
    await finalizePendingAccountResponse(completion)
  } catch (e: unknown) {
...
async function handleCreateAccount(payload: PendingOAuthCreateAccountPayload) {
  ...
  isSubmitting.value = true
  try {
    const { data } = await apiClient.post<CommunityPendingActionResponse>('/auth/oauth/pending/create-account', {
      email: payload.email,
...
async function handleBindLogin() {
  ...
  isSubmitting.value = true
  try {
    const { data } = await apiClient.post<CommunityPendingActionResponse>('/auth/oauth/pending/bind-login', {
      email,
      password,
      ...serializeAdoptionDecision(currentAdoptionDecision())
    })
...
onMounted(async () => {
  ...
  try {
    ...
    if (applyTotpChallenge(completion as CommunityPendingActionResponse)) {
      persistPendingAuthSession(completionRedirect)
      return
    }

    applyPendingAccountAction(completion as CommunityPendingActionResponse)
    if (pendingAccountAction.value !== 'none') {
      isProcessing.value = false
      persistPendingAuthSession(completionRedirect)
      return
