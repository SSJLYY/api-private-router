...
import CommunityCallbackView from '../CommunityCallbackView.vue'
...
const completeCommunityOAuthRegistration = vi.fn()
...
vi.mock('@/api/auth', async () => {
  ...
  return {
    ...
    completeCommunityOAuthRegistration: (...args: any[]) => completeCommunityOAuthRegistration(...args),
...
describe('CommunityCallbackView', () => {
  beforeEach(() => {
    replace.mockReset()
    ...
    completeCommunityOAuthRegistration.mockReset()
  ...
  it('accepts the legacy pending oauth invitation fragment without pending-session exchange', async () => {
    const { getCommunityOAuthCompleteRegistrationPath } = await import('@/api/auth')
    ...
    const wrapper = mount(CommunityCallbackView, {
      global: {
        stubs: {
          AuthLayout: { template: '<div><slot /></div>' },
          Icon: true,
    ...
    expect(apiClientPost).toHaveBeenCalledWith(getCommunityOAuthCompleteRegistrationPath(), {
  ...
  it('waits for explicit adoption confirmation before finishing a non-invitation login', async () => {
    exchangePendingOAuthCompletion
      .mockResolvedValueOnce({
        redirect: '/dashboard',
        adoption_required: true,
        suggested_display_name: 'Community Nick',
    ...
    expect(wrapper.text()).toContain('Community Nick')
  ...
  it('keeps rendering pending bind-login UI when adoption confirmation leads to another pending step', async () => {
    ...
    expect(replace).not.toHaveBeenCalled()
    expect((wrapper.get('[data-testid="community-bind-login-email"]').element as HTMLInputElement).value).toBe(
  ...
  it('renders adoption choices for invitation flow and submits the selected values', async () => {
    ...
    completeCommunityOAuthRegistration.mockResolvedValue({
      access_token: 'access-token',
      refresh_token: 'refresh-token',
      expires_in: 3600,
      token_type: 'Bearer'
    ...
    expect(completeCommunityOAuthRegistration).toHaveBeenCalledWith('invite-code', {
  ...
  it('collects email, password, and verify code for pending oauth account creation and submits adoption decisions', async () => {
    ...
    expect(checkboxes).toHaveLength(2)
    await checkboxes[1].setValue(false)
    await wrapper.get('[data-testid="community-create-account-email"]').setValue('  new@example.com  ')
    await wrapper.get('[data-testid="community-create-account-password"]').setValue('secret-123')
    await wrapper.get('[data-testid="community-create-account-verify-code"]').setValue('246810')
    await wrapper.get('[data-testid="community-create-account-invitation-code"]').setValue(' INVITE123 ')
    await wrapper.get('[data-testid="community-create-account-submit"]').trigger('click')
    await flushPromises()
  ...
  it('switches to bind-login when create-account returns EMAIL_EXISTS', async () => {
    ...
    await flushPromises()
    await wrapper.get('[data-testid="community-create-account-email"]').setValue('existing@example.com')
    await wrapper.get('[data-testid="community-create-account-password"]').setValue('secret-123')
    await wrapper.get('[data-testid="community-create-account-submit"]').trigger('click')
    await flushPromises()
  ...
  it('sends a verify code for pending oauth account creation', async () => {
    ...
    await wrapper.get('[data-testid="community-create-account-email"]').setValue('  new@example.com  ')
    await wrapper.get('[data-testid="community-create-account-send-code"]').trigger('click')
    await flushPromises()
  ...
  it('shows bind-login form for existing account binding and submits credentials with adoption decisions', async () => {
    ...
    expect(checkboxes).toHaveLength(2)
    await checkboxes[0].setValue(false)
    await wrapper.get('[data-testid="community-bind-login-email"]').setValue('existing@example.com')
    await wrapper.get('[data-testid="community-bind-login-password"]').setValue('secret-password')
    await wrapper.get('[data-testid="community-bind-login-submit"]').trigger('click')
    await flushPromises()
  ...
  it('handles bind-login 2FA challenge before redirecting', async () => {
    ...
    await wrapper.get('[data-testid="community-bind-login-totp"]').setValue('123456')
    await wrapper.get('[data-testid="community-bind-login-totp-submit"]').trigger('click')
    await flushPromises()
