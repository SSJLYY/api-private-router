...
vi.mock("vue-i18n", async () => {
  ...
  const translations: Record<string, string> = {
    ...
    "admin.settings.authSourceDefaults.requireEmailHint": "启用后，社区登录、OIDC、微信注册缺少邮箱时必须先补充邮箱地址。",
    ...
    "admin.settings.authSourceDefaults.sources.linuxdo.title": "社区登录",
    "admin.settings.authSourceDefaults.sources.linuxdo.description": "适用于社区第三方注册的新用户默认配额。",
    ...
    "admin.settings.payment.title": "支付系统选项",
    "admin.settings.payment.description": "支付系统选项",
    "admin.settings.payment.configGuideHint": "部署包文档中包含配置说明，建议优先选择结算稳定且回调能力完整的支付方式。",
...
describe("admin SettingsView payment visible method controls", () => {
  ...
  it("renders inline payment guidance without external repository links", async () => {
    const wrapper = mountView();

    await flushPromises();
    await openPaymentTab(wrapper);

    expect(wrapper.text()).toContain("支付系统选项");
    expect(wrapper.text()).toContain("部署包文档");
    expect(wrapper.text()).toContain("结算稳定且回调能力完整");
    expect(wrapper.html()).not.toContain("blob/main/docs/PAYMENT");
  });
